<template>
  <div>
    <v-dialog v-model="dialogModal" width="1024" transition="dialog-top-transition">
      <template v-slot:activator="{ props }">
        <div @click="(validation.value = {}), getDataCreate()" v-bind="props">
          ...
          <v-card-text>
            <v-container>
              <v-row>
                ...
                <v-col cols="6">
                  ...
                  <v-autocomplete
                    ...
                    @update:model-value="updateKategori"
                  >
                  </v-autocomplete>
                </v-col>
                <v-col cols="6">
                ...
                <v-col cols="12">
                  ...
                  <v-file-input
                    :messages="payload.file_buku ? payload.file_buku : ''"
                ...
                <v-col cols="12">
                  <div class="text-subtitle-1">Stok*</div>
                  <v-text-field
                    type="number"
                    density="compact"
                    ...
                    bg-color="white"
                    :error-messages="
                      validation.value?.stok_total ? 'The stok field is required.' : ''
                    "
              ...
              <v-card-actions>
                ...
                <v-btn color="primary" variant="flat" @click.prevent="tambahBuku">
                  Konfirmasi
                </v-btn>
              </v-card-actions>
            </v-container>
...
const tambahBuku = async () => {
  ...
  formData.append('stok_tersedia', payload.stok)
  formData.append('stok_total', payload.stok)
  ...
  try {
    ...
    if (response.data.success) {
      state.toasterMessage = 'Data buku berhasil ditambahkan'
      state.toaster = true
